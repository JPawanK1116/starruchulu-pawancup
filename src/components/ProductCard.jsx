        ...
        >
            {product.isBestSeller && (
                <div className="absolute top-4 -left-2 z-10">
                    <div className="bg-red-600 text-white text-xs font-bold px-3 py-1 rounded-r-md shadow-md uppercase tracking-wide">
                        🔥 Best Seller
                    </div>
            ...
            <div className="p-5 flex flex-col flex-grow">
                <div className="flex justify-between items-start mb-2">
                    <p className="text-xs font-semibold uppercase tracking-wider text-[var(--color-primary-green)]">
                        {product.category}
                    </p>
                ...
                <Link to={`/product/${product.id}`}>
                    <h3 className="text-lg font-heading font-bold text-[var(--color-dark-text)] mb-2 line-clamp-2 hover:text-[var(--color-primary-red)] transition-colors">
                        {product.name}
                    </h3>
                </Link>
                ...
                <div className="mt-auto pt-4 flex flex-col gap-3 border-t border-gray-100">
                    <div className="flex justify-between items-center bg-gray-50 rounded-lg p-1">
                        {Object.keys(product.pricePerWeight).map(weight => (
                            <button
                                ...
                                className={`py-1 px-2 md:px-3 text-xs md:text-sm font-medium rounded-md flex-1 transition-colors ${selectedWeight === weight
                    ...
                    <div className="flex items-center justify-between">
                        <span className="text-xl font-bold text-[var(--color-primary-red)]">
                            ₹{price}
                        </span>
                        <button
                            onClick={handleAddToCart}
                            className="md:hidden bg-[var(--color-primary-red)] text-white p-2.5 rounded-full hover:bg-red-800 active:scale-95 transition-all shadow-md flex items-center justify-center"
